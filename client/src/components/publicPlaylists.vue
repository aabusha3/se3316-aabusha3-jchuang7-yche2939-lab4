...
async function viewTracks(id){
  ...
  const tracksStr = arrToStr(result.value[id].tracks);
  ...
  <table>
    ...
    <tbody>
      <tr v-for="(list, index) in result" :key="index">
        ...
        <td>{{list.duration}}</td>
        <td>{{typeof list.comments==='undefined'? 0:(list.comments.length > 0? ((list.comments.map(e=>e.rating).reduce((accumulator, currentValue) => accumulator + currentValue) / list.comments.length).toFixed(2)) : 0)}} / 5</td>
        <td>
