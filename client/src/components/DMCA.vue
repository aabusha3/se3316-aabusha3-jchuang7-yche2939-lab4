...
function getPById(){
    if(inputNumber.value<1 || inputNumber.value>public_dmca.value.length){
        alert("Invalid Input!")
        smText.value = ""
        smID.value = null
    }
    else{
        smText.value = public_dmca.value[inputNumber.value-1].description
        smID.value = public_dmca.value[inputNumber.value-1]._id
}}
...
async function updatePolicy(){
    ...
    if(smText.value !== null){
        public_dmca.value[inputNumber.value-1].description = smText.value
    }
}
 </script>
